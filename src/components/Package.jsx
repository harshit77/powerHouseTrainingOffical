...
class Package extends React.Component {
  ...
  componentDidMount() {
    this.props.option != "Create New Package"
      ? this.setState({
          fees: this.props.dataForPackage.package[
            this.props.dataForPackage.package.length - 1
          ].fees,
        })
      : null;
  }
  handleFees(event) {
    event.target.value != 0
      ? this.setState({ feesError: null, fees: event.target.value })
      : this.setState({
          feesError: "First Name is required",
  ...
  handleselectDate(date, datePicker) {
    ...
    datePicker === "StartDate"
      ? this.setState({ datePickerLabel: date })
      : this.setState({ datePickerLabelEnd: date, endDateError: null });
  }
  ...
  handlePackage() {
    ...
    endDate =
      ...
      endDate.getFullYear();
    if (!this.state.fees)
      this.setState({ feesError: "Fees field is required" });
    if (startDate == endDate)
      this.setState({ endDateError: "Start and End Date can not be Same" });
    if (
      !this.state.fees ||
      !this.state.datePickerLabel ||
      !this.state.datePickerLabelEnd ||
      this.state.endDateError != null ||
      this.state.feesError != null ||
      startDate == endDate
    )
      return;
    else {
      ...
      this.props.option != "Create New Package"
        ? this.props
            .updateMemberPackage({
              variables: {
                ...
                previousPackageId: this.props.dataForPackage.package[
                  this.props.dataForPackage.package.length - 1
                ].id,
              },
            })
        ...
        : this.props
            .createPackage(
              this.state.filterOptions.typeEnabled,
              this.props.dataForPackage.id,
              this.state.datePickerLabel,
              this.state.datePickerLabelEnd,
              this.state.fees
            )
  ...
  render() {
    return (
      <React.Fragment>
        {this.props.dataForPackage != null && (
            ...
            <Paper className="paperHeader" elevation="14">
              <div>
                {this.props.option == "Create New Package"
                  ...
                  : "Edit Package"}{" "}
                for
                {this.state.globalStatus == "Package created successfully" ||
                this.state.globalStatus == "Package updated successfully" ? (
                  <Link to={`/userDetails/${this.props.dataForPackage.id}`}>
                    <strong>
                      {" "}
                      {this.props.dataForPackage.firstName}{" "}
                      {this.props.dataForPackage.lastName}
                    </strong>
                  </Link>
                ) : (
                  <strong>
              ...
              <div className="fieldAligment">
                <div style={{ marginLeft: 10 }}>
                  <CustomRadioButton
                    radioSelected={this.handleFilter}
                    radioOption={filterOptions}
                    selected={this.state.filterOptions.typeEnabled}
                  />
                </div>
                {this.props.option != "Create New Package" ? (
                  <div className="dateSearchContainer marginLeftText">
                    <span className="messageALert">
                      Previous Start Date :{" "}
                      {
                        this.props.dataForPackage.package[
                          this.props.dataForPackage.package.length - 1
                        ].startDate
                      }
                    </span>
                    <div className="dateBoxSepartor"> </div>
                    <span className="messageALert" style={{ marginLeft: "2%" }}>
                      Previous End Date :{" "}
                      {
                        this.props.dataForPackage.package[
                          this.props.dataForPackage.package.length - 1
                ...
                <div className="dateSearchContainer marginLeftText marginTopText">
                  ...
                  <CustomDatePicker
                    labelName="End Date"
                    className="halfWidth"
                    minDate={this.state.datePickerLabel}
                    disableFuture={false}
                    selectDate={this.handleselectDate}
                  />
                  {this.state.endDateError != null ? (
                    <div className="error marginLeftText">
                      Start and End Date can not be Same
                    </div>
                  ) : null}
                </div>
                <TextField
                ...
                <div className="halfWidth fieldAligment">
                  <Button
                    ...
                    className="buttonSearch noMarginLeft fullWidth"
...
const PackageWithData = compose(
  graphql(query, {
    props: ({ ownProps, mutate }) => ({
      createPackage: (name, memberShipId, startDate, endDate, fees) =>
        mutate({
